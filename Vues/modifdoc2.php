                    ...
                    <div id="content" class="site-content" >
                        <main id="main">
                            ...
                            <div class="page-grid" >
                                ...
                                $us=$doc->aff_doc($_GET['m']);
                                extract($us);
                                ?>
                                <form method="post" enctype="multipart/form-data" >
                                <!-- GRID 9 COLUMNS -->
                                <h2 class="section-header">modifier le document " <?php echo $nom ?> "</h2>
                                <label>Nom</label> 
                               ...
                               if(isset($_POST['modif'])){
                                ...
                                copy($_FILES['photo0']['tmp_name'],'documents/conception/'.$_FILES['photo0']['name']);
                                $fichier1 = 'documents/conception/'.$Fichier;
                                unlink( $fichier1 ) ;
                                }

                                if ($plateforme=='SB') {
                                $photo2=$_FILES['photo0']['name'];
                                copy($_FILES['photo0']['tmp_name'],'documents/SB/'.$_FILES['photo0']['name']);
                                $fichier2 = 'documents/SB/'.$Fichier;
                                unlink( $fichier2 ) ;
                                }
                                if ($plateforme=='micro') {
                                $photo2=$_FILES['photo0']['name'];
                                copy($_FILES['photo0']['tmp_name'],'documents/micro/'.$_FILES['photo0']['name']);
                                $fichier3 = 'documents/micro/'.$Fichier;
                                unlink( $fichier3 ) ;
                                }
                                if ($plateforme=='carac') {
                                $photo2=$_FILES['photo0']['name'];
                                copy($_FILES['photo0']['tmp_name'],'documents/carac/'.$_FILES['photo0']['name']);
                                $fichier4 = 'documents/carac/'.$Fichier;
                                unlink( $fichier4 ) ;
                                }
                                if ($plateforme=='ob') {
                                $photo2=$_FILES['photo0']['name'];
                                copy($_FILES['photo0']['tmp_name'],'documents/objets/'.$_FILES['photo0']['name']);
                                $fichier5 = 'documents/objets/'.$Fichier;
                                unlink( $fichier5 ) ;
                                }
                                if ($plateforme=='nano') {
                                $photo2=$_FILES['photo0']['name'];
                                copy($_FILES['photo0']['tmp_name'],'documents/nano/'.$_FILES['photo0']['name']);
                                $fichier6 = 'documents/nano/'.$Fichier;
                                unlink( $fichier6 ) ;
                                }
                                if ($plateforme=='bio') {
                                $photo2=$_FILES['photo0']['name'];
                                copy($_FILES['photo0']['tmp_name'],'documents/bio/'.$_FILES['photo0']['name']);
                                $fichier7 = 'documents/bio/'.$Fichier;
                                unlink( $fichier7 ) ;
                                }
                                if ($plateforme=='hyper') {
                                $photo2=$_FILES['photo0']['name'];
                                copy($_FILES['photo0']['tmp_name'],'documents/hyper/'.$_FILES['photo0']['name']);
                                $fichier8 = 'documents/hyper/'.$Fichier;
                                unlink( $fichier8 ) ;
                                }
                                ...
                                $doc->modif_doc($_GET['m'],$_POST['nom0'],$photo2);  
